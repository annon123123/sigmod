...
if [ $? -ne 0 ]; then
  ...
  pushd tpcds-gen > /dev/null
    hadoop jar target/*.jar -Ddfs.replication=1 -d ${DIR}/${SCALE}/ -s ${SCALE}
    if [ $? -ne 0 ]; then
      echo "MR job failed, please remove ${DIR}/${SCALE} and try again. Data generation failed, exiting."
      exit 1
    fi
  popd > /dev/null
fi
...
for t in ${DIMS}
do
  COMMAND="hive -i settings/load-partitioned.sql --hiveconf hive.log.file=hive-gen-${t}.log -f ddl-tpcds/bin_partitioned/${t}.sql \
