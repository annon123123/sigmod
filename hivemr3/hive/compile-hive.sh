...
function compile_hive_print_usage {
    ...
    echo ""
    echo "Example: ./compile-hive.sh --hivesrc3 -pl ql -pl common"
    echo ""
}
...
function compile_hive_main {
    ...
    compile_hive_parse_args $REMAINING_ARGS
    ...
    mvn install:install-file -Dfile=$MR3_TEZ_JAR_PATH -DgroupId=mr3 -DartifactId=mr3-tez -Dversion=$MR3_REV -Dpackaging=jar 2>&1 | tee -a $COMPILE_OUT_FILE
    if [ $? -ne 0 ]; then
