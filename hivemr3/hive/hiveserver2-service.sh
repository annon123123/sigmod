...
function hiveserver2_service_init {
    ...
    echo -e "\n# Running HiveServer2 using Hive-MR3 ($HIVE_REV, $TEZ_REV) #\n" >&2
    # set up an output directory, e.g. stores all configuration files
...
function start_hiveserver2 {
    ...
    if [[ $SYNC_TIME = true ]]; then
        common_setup_sync_time $LOCAL_MODE $hosts_file
    fi
    if [[ $CLEAR_CACHE = true ]]; then
        common_setup_clear_cache $LOCAL_MODE $hosts_file
    fi
    if [[ $PROFILE = true ]]; then
        common_setup_clear_profiler $LOCAL_MODE $hosts_file
    fi
    if [[ $ENABLE_DSTAT = true ]]; then
        common_setup_start_dstat $LOCAL_MODE $hosts_file $OUT
    fi
    if [[ $ENABLE_SS = true ]]; then
        common_setup_start_ss $LOCAL_MODE $hosts_file $OUT
    fi
...
function stop_hiveserver2 {
    ...
    hosts_file=$OUT_CONF/slaves
    if [[ $ENABLE_DSTAT = true ]]; then
        common_setup_collect_dstat $LOCAL_MODE $hosts_file $OUT
        common_setup_clean_stat $LOCAL_MODE $hosts_file $OUT
    fi
    if [[ $ENABLE_SS = true ]]; then
        common_setup_collect_ss $LOCAL_MODE $hosts_file $OUT
        common_setup_clean_stat $LOCAL_MODE $hosts_file $OUT
    fi
    if [[ $PROFILE = true ]] && [[ $START_HIVE_SERVER2 = false ]]; then
        echo "Collecting profiler results is disabled"
        # common_setup_get_profiler_report $LOCAL_MODE $hosts_file $OUT/profile
    fi
...
function main {
    ...
    # Delete out dirs only if HiveServer2 successfully started
