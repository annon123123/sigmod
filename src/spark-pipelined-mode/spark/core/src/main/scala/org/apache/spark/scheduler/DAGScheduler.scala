...
private[spark] class DAGScheduler(
  ...
  // This map tracks the relation between jobId and stages contained in the corresponding DAG.
  private[scheduler] val jobIdToSubmittedStages = new HashMap[Int, HashSet[Stage]]

  private[this] var waitingJobs = Seq.empty[ActiveJob]
  ...
  private def cleanupStateForJobAndIndependentStages(job: ActiveJob): Unit = {
    ...
    waitingJobs = waitingJobs.filterNot(_ == job)
    jobIdToSubmittedStages -= job.jobId
  ...
  private def createVertexPayload(jobId: Int, stage: Stage): VertexPayload = {
    stage match {
    ...
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = stage match {
      case sms: ShuffleMapStage =>
        partitionsToCompute.map { id => (id, getPreferredLocs(stage.rdd, id))}.toMap
      case rs: ResultStage =>
        partitionsToCompute.map { id =>
          val p = rs.partitions(id)
          (id, getPreferredLocs(stage.rdd, p))
        }.toMap
    }

    val numTask = partitionsToCompute.length
    stage.makeNewStageAttempt(numTask, partitionsToCompute.map(taskIdToLocations))
    ...
    stage match {
      case sms: ShuffleMapStage =>
        sms.pendingPartitions.clear()
        sms.pendingPartitions ++= partitionsToCompute
      case rs: ResultStage =>
    }
    ...
    val taskBinary = sc.broadcast(taskBinaryBytes)
    val serializedTaskMetrics = closureSerializer.serialize(stage.latestInfo.taskMetrics).array()

    VertexPayload(stage, taskBinary, serializedTaskMetrics, properties,
      partitionsToCompute, partitions, taskIdToLocations)
  }

  private def createDAGs(finalStage: Stage): (Seq[Seq[Stage]], Boolean) = {
    if (runningStages.contains(finalStage)) {
      (Seq.empty, false)
    } else {
      val isPartiallyComputed = finalStage match {
        case sms: ShuffleMapStage =>
          sms.numAvailableOutputs != 0
        case rs: ResultStage =>
          false
      }

      var parentsDags: Seq[Seq[Stage]] = Seq.empty
      var isAllParentsExtensible = true
      getMissingParentStages(finalStage)
        .map { createDAGs }
        .foreach { case (dags, isExtensible) =>
          isAllParentsExtensible &= isExtensible
          parentsDags ++= dags
        }

      if (isAllParentsExtensible) {
        (Seq(parentsDags.flatten :+ finalStage), !isPartiallyComputed)
      } else {
        (parentsDags, false)
      }
    }
  ...
  private def submitActiveJob(activeJob: ActiveJob): Unit = {
    val finalStage = activeJob.finalStage
    val stages = createDAGs(finalStage)._1.flatten.distinct
    if (stages.nonEmpty) {
      val jobId = activeJob.jobId
      val stagesToBeSubmitted = new HashMap[Int, Stage]
      val stageIdToTaskBinary = new HashMap[Int, Broadcast[Array[Byte]]]
      val stageIdToTaskMetrics = new HashMap[Int, Array[Byte]]
      val stageIdToProperties = new HashMap[Int, Properties]
      val stageIdToPartitions = new HashMap[Int, Array[Partition]]
      val stageIdToPartitionIndexToCompute = new HashMap[Int, Seq[Int]]
      val stageIdToTaskLocationHints = new HashMap[Int, Map[Int, Seq[TaskLocation]]]

      val dagSubmitted = try {
        val vertexPayloads = stages.map { stage => createVertexPayload(jobId, stage) }
        assert { vertexPayloads.forall { _.partitionsToCompute.nonEmpty } }

        vertexPayloads.foreach { vertexPayloads =>
          val stageId = vertexPayloads.stage.id
          stagesToBeSubmitted(stageId) = vertexPayloads.stage
          stageIdToTaskBinary(stageId) = vertexPayloads.taskBinary
          stageIdToTaskMetrics(stageId) = vertexPayloads.serializedTaskMetrics
          stageIdToProperties(stageId) = vertexPayloads.properties
          stageIdToPartitions(stageId) = vertexPayloads.partitions
          stageIdToPartitionIndexToCompute(stageId) = vertexPayloads.partitionsToCompute
          stageIdToTaskLocationHints(stageId) = vertexPayloads.taskIdToLocations
        }

        taskScheduler.submitJob(
          activeJob, stagesToBeSubmitted, stageIdToTaskBinary, stageIdToTaskMetrics,
          stageIdToProperties, stageIdToPartitions, stageIdToPartitionIndexToCompute,
          stageIdToTaskLocationHints)
        true
      } catch {
        case th: Throwable =>
          cleanupStateForJobAndIndependentStages(activeJob)
          val error =
            new SparkException(s"Failed to submit a DAG for Job (${activeJob.jobId})", th)
          activeJob.listener.jobFailed(error)
          listenerBus.post(
            SparkListenerJobEnd(activeJob.jobId, clock.getTimeMillis(), JobFailed(error)))
          false
      }

      if (dagSubmitted) {
        jobIdToSubmittedStages.put(activeJob.jobId, new HashSet)
        stages.foreach { stage =>
          val properties = stageIdToProperties(stage.id)
          stage.latestInfo.submissionTime = Some(clock.getTimeMillis())
          listenerBus.post(
            SparkListenerStageSubmitted(stage.latestInfo, Utils.cloneProperties(properties)))
          runningStages += stage
          jobIdToSubmittedStages(activeJob.jobId) += stage
        }

        if (!stages.contains(finalStage)) {
          logInfo(s"Job ${activeJob.jobId} is added to waitingJobs" +
            "because DAGScheduler submits a small DAG to compute incomplete MapOutput.")
          waitingJobs ++= Seq(activeJob)
        }
      }
    } else {
      logInfo(s"Job ${activeJob.jobId} is added to waitingJobs " +
        "because another DAG is computing some of its parent stages.")
      waitingJobs ++= Seq(activeJob)
    }
  }

  private def submitWaitingJobs(): Unit = {
    val copiedWaitingJobs = waitingJobs
    waitingJobs = Seq.empty[ActiveJob]

    copiedWaitingJobs.foreach { submitActiveJob }
  }
  ...
  private[scheduler] def handleTaskCompletion(event: CompletionEvent): Unit = {
    ...
    event.reason match {
      ...
      case FetchFailed(bmAddress, shuffleId, _, mapIndex, reduceId, failureMessage) =>
        // MR3 handles FetchFailure, including rerun of Map vertex.
        // Do we have to unregister MapStatus/MergeStatus?

      case failure: TaskFailedReason if task.isBarrier =>
        // MR3 should handle the failure of BarrierTask.
        // Currently, MR3 does not fully support BarrierExecution.

      case Resubmitted =>
        // MR3Backend does not report Resubmitted. So skip calling the following method.
  ...
  private def processShuffleMapStageCompletion(shuffleStage: ShuffleMapStage): Unit = {
    ...
    if (!shuffleStage.isAvailable) {
      ...
      // TODO: We should either stop running the Job or resubmit it.
    } else {
  ...
  private def cancelRunningIndependentStages(job: ActiveJob, reason: String): Boolean = {
    ...
    taskScheduler.killJob(job.jobId)
    ...
    stages.foreach { stageId =>
      ...
      if (jobsForStage.isEmpty || !jobsForStage.get.contains(job.jobId)) {
        ...
        if (!stageIdToStage.contains(stageId)) {
          ...
          if (runningStages.contains(stage)) {
            try {
              // call killJob() instead of cancelTasks() to stop running the submitted DAG
