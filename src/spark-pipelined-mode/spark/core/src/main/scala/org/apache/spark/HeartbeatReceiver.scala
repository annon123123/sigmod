    ...
    for ((executorId, lastSeenMs) <- executorLastSeen) {
      if (now - lastSeenMs > executorTimeoutMs) {
        ...
        killExecutorThread.submit(new Runnable {
          // Spark-MR3 fails the below code because vanilla Spark does not know MR3Backend.
          // Since this thread just emits an error message and stops in that case,
          // we don't fix it as of now.
          override def run(): Unit = Utils.tryLogNonFatalError {
