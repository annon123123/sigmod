...
private[spark] class Executor(
  ...
  def launchTask(
      ...
      isBarrierTask: Boolean,
      isResultTask: Boolean,
      outputId: Option[Int],
      serializedTaskBinary: ByteBuffer,
      serializedPartition: ByteBuffer,
      serializedTaskMetrics: Array[Byte]): Unit = {
    val tr = new TaskRunner(context, taskDescription, stageId = stageId,
      stageAttemptId = stageAttemptId, jobId = jobId, appId, appAttemptId,
      isBarrierTask = isBarrierTask, isResultTask = isResultTask, outputId,
      serializedTaskBinary = serializedTaskBinary, serializedPartition = serializedPartition,
      serializedTaskMetrics, plugins)
  ...
  class TaskRunner(
      ...
      private val stageAttemptId: Int,
      private val jobId: Int,
      private val appId: String,
      private val appAttemptId: Option[String],
      private val isBarrierTask: Boolean,
      private val isResultTask: Boolean,
      private val outputId: Option[Int],
      private val serializedTaskBinary: ByteBuffer,
      private val serializedPartition: ByteBuffer,
      private val serializedTaskMetrics: Array[Byte],
    ...
    def this(
      ...
      // Spark-MR3 with pipelined shuffle never calls this constructor.
      // We keep this constructor just for the sake of compilation.
      this(execBackend, taskDescription, 0, 0, 0, "", None, false, false, None, null, null,
        Array.empty, None)
    }
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        val taskBinary = ser.deserialize[Broadcast[Array[Byte]]](
          serializedTaskBinary, Thread.currentThread.getContextClassLoader)
        val partition = ser.deserialize[Partition](
          serializedPartition, Thread.currentThread.getContextClassLoader)
        task = if (isResultTask) {
          assert { outputId.isDefined }
          new ResultTask(
            stageId = stageId,
