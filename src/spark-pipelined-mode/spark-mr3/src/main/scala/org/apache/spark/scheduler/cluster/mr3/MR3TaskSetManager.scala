...
private[mr3] object MR3TaskSetManagerState extends Enumeration {
  ...
  // Available state transition: Running => {Succeeded, Failed}
...
private[mr3] class MR3TaskSetManager(
  ...
  //
  // stateLock guards state, taskSucceededFlags, numSucceededTasks, firstAttemptTaskInfos, and taskInfoMap
  //
  ...
  // DAGScheduler: Running -> {Stopped, Failed}
  // DAGStatusChecker: Running -> Failed
  // MR3TaskResultGetter: Running -> Succeeded
  // A thread that escapes Running state should call notifyAllTasksKilled().
  @GuardedBy("stateLock")
  ...
  // We can dispense with stateLock.synchronized{} in notifyAllTasksKilled():
  //   1) notifyAllTasksKilled() is called by the thread that escapes Running state.
  //   2) once notifyAllTasksKilled() is called, no other threads access firstAttemptTaskInfos[] and taskInfoMap[].
  @GuardedBy("stateLock")
  ...
  private def notifyAllTasksKilled(): Unit = {
    ...
    val (isTaskSucceeded, transitionToSucceeded) = stateLock.synchronized {
      ...
      val isTaskSucceeded =
        if (curState == Running) {
          ...
          if (numSucceededTasks >= numTasks && (taskSucceededFlags forall { _ == true })) {
            state = Succeeded
          }
          true
        } else {
  ...
  def kill(message: String): Unit = {
    val transitionToFailed = stateLock.synchronized {
      if (state == Running) {
        state = Failed
        true
      } else
  ...
  // the caller method should kill this TaskSet and the corresponding Job.
