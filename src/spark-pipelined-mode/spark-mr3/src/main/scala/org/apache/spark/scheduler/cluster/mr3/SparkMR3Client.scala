...
import mr3.DAGAPI
import mr3.api.client.{DAGClient, MR3SessionClient, SessionStatus}
import mr3.api.common.{MR3Conf, MR3Constants, MR3Exception, Utils => MR3Utils}
import mr3.common.{Utils => MR3CommonUtils}
...
private[mr3] class SparkMR3Client(
  ...
  def submitJob(
      ...
      sparkStageInfoProtoMap: Map[Int, DAGAPI.SparkStageInfoProto.Builder],
      partitionAndTaskLocationMap: Map[Int, Seq[(DAGAPI.SparkPartitionInfoProto.Builder, Seq[String])]],
      sparkConf: SparkConf,
      dagConf: DAGAPI.ConfigurationProto.Builder,
      credentials: Option[Credentials]): DAGClient = {
    val modifiedPartitionAndTaskLocationMap = mr3Conf.getAmWorkerMode match {
      case MR3Constants.MR3_WORKER_MODE_YARN =>
        partitionAndTaskLocationMap.mapValues { partitionAndTaskLocationList =>
          partitionAndTaskLocationList.map { case (partition, hints) =>
            val modifiedHints = hints.map { address => MR3CommonUtils.hostAddressToHostName(address) }
            (partition, modifiedHints)
          }
        }
      case MR3Constants.MR3_WORKER_MODE_KUBERNETES =>
        partitionAndTaskLocationMap.mapValues { partitionAndTaskLocationList =>
          partitionAndTaskLocationList.map { case (partition, hints) =>
            val modifiedHints = hints.flatMap { host => MR3CommonUtils.hostNameToHostAddress(host) }
            (partition, modifiedHints)
          }
        }
      case _ =>
    ...
    val dagProto =
      BigDAG.createDag(
        applicationId = sparkApplicationId,
        jobId,
        sparkStageMetaDataMap,
        sparkStageInfoProtoMap,
        modifiedPartitionAndTaskLocationMap,
        driverUrl = driverUrl,
        sparkConf,
        dagConf,
        credentials,
